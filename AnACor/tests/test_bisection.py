...
class TestBasic:
    ...
    @pytest.mark.order(4)
    # @pytest.mark.skip(reason="This Bisection test is being skipped.")
    def test_Bisection(self):
        with warnings.catch_warnings():
            ...
            logger.info("======== test_Bisection starts========")
            t1 = time.time()
            low = 0
            dataset = 'pytest_Bisection'
            mu=0.01
            ...
            logger.info(print_out)
            logger.info("It takes {:.5f} seconds to run Bisection ray-tracing test".format(t2-t1))
            logger.info("\n")
            logger.info("========test_Bisection is done========")
            logger.info("\n")
            # pdb.set_trace()